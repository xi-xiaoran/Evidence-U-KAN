...
from Bys_train import train_model
from Bys_test import evaluate_model
...
if Train:
    ...
    for train_index, val_index in kf.split(dataset):
        ...
        model = train_model(model, train_loader, val_loader, criterion, optimizer, device,
                            num_epochs=num_epochs, fold=num)
        ...
        for std in range(0, 5, 1):
            std = (std * 1.0 / 10)
            Dice, Iou, Assd, ECE, UEO, T_U, F_U, best_T_U, best_F_U, worst_T_U, worst_F_U = evaluate_model(model, test_loader, device,
                                                                                     uncertainty, std)
            print('std', std)
            print('Dice', Dice)
            print('Iou', Iou)
            ...
            plt.title('Bayes', color='darkred')
            plt.savefig(f'{num} Bayes Uncertainty distribution.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            plt.ylabel('Density', color='black')
            plt.title('Bayes', color='darkred')
            plt.savefig(f'{num} Bayes Best Uncertainty distribution.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            plt.ylabel('Density', color='black')
            plt.title('Bayes', color='darkred')
            plt.savefig(f'{num} Bayes Worst Uncertainty distribution.png', dpi=300, bbox_inches='tight')
            plt.close()
