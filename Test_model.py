...
# Evaluate model
def evaluate_model(model, test_loader, device, uncertainty, likehood=0.3, std=0):
    # model.eval()
    model.train()
    num = 0
    dice = 0
    ...
    best_F_U = np.array([])
    worst_F_U = np.array([])
    all_u = torch.tensor([]).to(device)
    for inputs, labels in test_loader:
        num = num + 1
        with torch.no_grad():
        # if True:
            inputs, labels = inputs.to(device), labels.to(device)
            inputs = add_noise(inputs, mean=0, stddev=std, device=device)
            ...
            if uncertainty:
                ...
                dice = dice + Dice(outputs, labels)
                iou = iou + Iou(outputs, labels)
                assd = assd + assd_score(outputs, labels)
                ece = ece + ECE_score(Belief, labels.unsqueeze(1))
                wucha = outputs.long() ^ labels.long()
                e = wucha.squeeze(0).squeeze(0)
                e = e.cpu().numpy()
                U = d
                ueo = ueo + Dice(U, e)
                dice1 = Dice(outputs, labels)
                dice1 = dice1.cpu().numpy()
                iou1 = Iou(outputs, labels)
                iou1 = iou1.cpu().numpy()
                assd1 = assd_score(outputs, labels)
                ece1 = ECE_score(Belief, labels.unsqueeze(1))
                ece1 = ece1.cpu().numpy()
                ueo1 = Dice(U, e)
                ...
                new_F_U = U[diff == 1]
                if len(T_U) <= 100000:
                    T_U = np.concatenate((T_U, new_T_U))
                if len(F_U) <= 100000:
                    F_U = np.concatenate((F_U, new_F_U))
                if ueo1 > best_ueo:
                    best_ueo = ueo1
                    best_T_U = new_T_U
                    best_F_U = new_F_U
                ...
                print(f'{num}/{len(test_loader)} ','Dice:',dice1,'Iou:',iou1,'ASSD:',assd1,'ECE:',ece1,'UEO:',ueo1)
